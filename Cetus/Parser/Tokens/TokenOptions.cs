...
public class TokenOptions(IToken[] possibleTokens) : IToken
{
	...
	public IToken Contextualize(IHasIdentifiers context, Args arguments, int order) => new TokenOptions(possibleTokens.Select(token => token.Contextualize(context, arguments, order)).ToArray());
